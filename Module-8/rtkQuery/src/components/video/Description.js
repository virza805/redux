...
export default function Description({videoDec}) {
    const {title, date, id, description} = videoDec;
    ...
    return (
        <div>
            ...
            {!isLoading && isError && (<Error message="There was an error for delete video!" />) }
                {description}
            </div>
        </div>
    );
